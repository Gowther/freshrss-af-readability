...
- Extracts Discourse topic links through their topic RSS feed before falling back to crawler HTML, JSON-LD, and Readability
...
Discourse topic URLs such as `/t/topic/12345` are handled by fetching `/t/topic/12345.rss` and combining the topic posts. If the topic RSS cannot be fetched or parsed, the extension tries Discourse's crawler HTML and JSON-LD structured data before falling back to the generic Readability extraction path. The RSS parser includes a lenient fallback for Discourse feeds whose XML cannot be parsed strictly.
